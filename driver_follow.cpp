/***************************************************************************
	...
	modified by Y at  April/21/2019 18:19
...
static void userDriverSetParam(float* cmdAcc, float* cmdBrake, float* cmdSteer, int* cmdGear) {
	...
	*cmdAcc = *cmdBrake = 0;//5,0,3
	...
	if (SpeedDown) {
		if (_speed < 130) {
			...
			else if (-60 < leaderAcc)offset = 5;
			else if (-75 < leaderAcc) offset = 8;
			else offset = 10;
		}
		else if (_speed < 150) {
	...
	if (-80 > leaderAcc && _speed > 100) { *cmdAcc = 0; *cmdBrake = 1; }		

	if (_Leader_Y < 10) { *cmdAcc /= 4; *cmdBrake *= 4; }
	if (_Leader_Y > 25) { *cmdAcc = 1; *cmdBrake = 0; }				
	...
	*cmdSteer = 1 * constrain(-1.0, 1.0, kp_dr * Dr_err + ki_dr * Dr_errSum + kd_dr * Dr_errDiff);
	...
	if (fabs(*cmdSteer) > 0.5) {
		//if(leaderAcc > 10 && last1leaderAcc > 10 && last2leaderAcc > 10){ *cmdAcc /= 1; *cmdBrake /= 2; }
		if (_speed < 125) { *cmdAcc /= 4; *cmdBrake += 0.01; }		//7&24
